...
"%JAVA%" -Xmx3G -jar "%JAR%" --inputDirectory "%SRC%" --outputFormat JAVA_1_12_2 --outputDirectory "%T%\world" --shiftToFit > "%T%\p1.log" 2>&1
echo [%TIME%] PASS1 exit=%ERRORLEVEL% >> "%LOG%"

echo [%TIME%] PASS2 same settings again >> "%LOG%"
"%JAVA%" -Xmx3G -jar "%JAR%" --inputDirectory "%SRC%" --outputFormat JAVA_1_12_2 --outputDirectory "%T%\world" --shiftToFit > "%T%\p2.log" 2>&1
...
"%JAVA%" -Xmx3G -jar "%JAR%" --inputDirectory "%SRC%" --outputFormat JAVA_1_12_2 --outputDirectory "%T%\world" --shiftToFit --blockMappings "%T%\one.json" > "%T%\p3.log" 2>&1
