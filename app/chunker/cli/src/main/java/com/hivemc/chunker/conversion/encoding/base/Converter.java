...
public interface Converter {
    ...
    /**
     * Record that one block instance had to be discarded because no mapping existed for it.
     * <p>
     * {@link #logMissingMapping(MissingMappingType, String)} answers "which blocks are missing?", which is what a
     * list of mappings needs, and it deliberately deduplicates. It therefore cannot answer "how much of the build is
     * gone?" - which is the question that decides which blocks are worth mapping by hand. This counts instances.
     *
     * @param identifier the block identifier which could not be written.
     */
    default void countUnmappedBlockInstance(String identifier) {
        // Optional detail: only a converter that produces a report needs to keep it.
    }

    /**
     * Record that a block was written as something other than itself.
     * <p>
     * Substituting an equivalent is the alternative to dropping a block, and it changes the build in a way the user
     * has to be told about - "my warped stairs are now nether brick" is something they may well want to overrule.
     * Counting these turns the mapping table from a list of the tool's opinions into a statement about the world in
     * front of them.
     *
     * @param source the block as it appears in the source world.
     * @param target what it was actually written as.
     */
    default void countSubstitution(String source, String target) {
        // Optional detail: only a converter that produces a report needs to keep it.
    }
