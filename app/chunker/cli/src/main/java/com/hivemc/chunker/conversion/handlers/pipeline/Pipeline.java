...
public class Pipeline {
    ...
    /**
     * Add an additional column handler on top of whatever is already configured, instead of replacing it.
     * <p>
     * Note the difference from {@link #columnHandlers}: that method sets the whole chain, so calling it twice
     * silently discards the first call. This one wraps the existing chain, which is what an extra step needs -
     * replacing the chain would drop stages that are already registered, such as the pre-transform pass that
     * decides how neighbouring blocks connect.
     *
     * @param handler the handler to wrap the existing chain with. It sees each column first and delegates onwards.
     */
    public final void addColumnHandler(BiFunction<ColumnConversionHandler, ChunkerWorld, ColumnConversionHandler> handler) {
        BiFunction<ColumnConversionHandler, ChunkerWorld, ColumnConversionHandler> previous = columnConversionTransformer;
        if (previous == null) {
            columnConversionTransformer = handler;
        } else {
            columnConversionTransformer = (delegate, world) -> handler.apply(previous.apply(delegate, world), world);
        }
    }
