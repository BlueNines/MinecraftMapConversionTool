...
public class JavaResolversBuilder {
    ...
    public JavaResolvers build() {
        ...
        return new JavaResolvers() {
            ...
            public LegacyIdentifier writeLegacyBlockIdentifier(ChunkerBlockIdentifier chunkerBlockIdentifier) {
                // Record the substitution here, where both halves are still known. By the time this returns, the
                // original block type has been replaced by whatever was written, and "your warped stairs are now
                // nether brick" is no longer recoverable from the output. A preserved identifier set by the reader
                // is the mark of a mapped block: it is what the writer is told to use instead.
                PreservedIdentifier preserved = chunkerBlockIdentifier.getPreservedIdentifier();
                if (preserved != null && preserved.fromReader()) {
                    converter.countSubstitution(
                            describe(chunkerBlockIdentifier.getType()),
                            preserved.identifier().getIdentifier()
                    );
                }
                ...
                return resolveLegacyBlockIdentifier(chunkerBlockIdentifier)
                        .orElseGet(() -> {
                            ...
                            // This runs once per block that is placed, so what it counts is the number of blocks
                            // actually lost rather than the number of distinct missing mappings. Air is excluded
                            // because it is not a loss, and it would otherwise inflate every total.
                            if (!chunkerBlockIdentifier.isAir()) {
                                converter.countUnmappedBlockInstance(String.valueOf(chunkerBlockIdentifier));
                            }
    ...
    /**
     * Turn a block type into a name a person would recognise.
     * <p>
     * The enum constant reads as WARPED_STAIRS, which is fine for a log line but not for a list the user has to scan;
     * the namespaced lowercase form is the same one they would type into a mappings file.
     *
     * @param type the block type.
     * @return the identifier form of the type.
     */
    private static String describe(com.hivemc.chunker.conversion.intermediate.column.chunk.identifier.type.block.ChunkerBlockType type) {
        if (type == null) return "unknown";
        String name = type.toString();
        return name.contains(":") ? name : "minecraft:" + name.toLowerCase(java.util.Locale.ROOT);
    }
}
