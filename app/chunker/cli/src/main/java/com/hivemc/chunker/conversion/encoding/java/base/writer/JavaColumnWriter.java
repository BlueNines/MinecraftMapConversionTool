...
public class JavaColumnWriter implements ColumnWriter {
     ...
     * <p>
     * The tags arrive in whatever order the async writing tasks happened to finish, which varies from run to run. The
     * fields are therefore sorted by name before being written.
     * <p>
     * This matters beyond tidiness. Two conversions of the same world with the same settings produce identical data,
     * so a re-conversion after editing one mapping ought to leave almost every chunk untouched - and the preview
     * relies on exactly that, re-rendering only the chunks whose bytes changed. With the field order left to the
     * scheduler, every chunk differed from its previous self on every run, so every chunk counted as changed and the
     * whole map was re-rendered even when a single block had moved. Sorting makes the output of a given world
     * reproducible, which is what makes "unchanged" detectable at all.
     *
    ...
    protected void combineNBT(CompoundTag output, List<TagWithName<?>> inputs) {
        List<TagWithName<?>> ordered = new ArrayList<>(inputs.size());
        for (TagWithName<?> input : inputs) {
            if (input != null) ordered.add(input);
        }
        ordered.sort(java.util.Comparator.comparing(TagWithName::name));
        for (TagWithName<?> input : ordered) {
