...
public class JavaChunkWriter {
    ...
    public CompoundTag writeChunk(@Nullable ChunkerChunk chunkerChunk) {
        ...
        Task.join(tasks).then("Combining Chunk NBT", TaskWeight.LOW, () -> {
            // The two writing tasks append to a shared list as they finish, so the order the fields land in depends
            // on which task the scheduler runs first - which varies between runs. Sorting by name makes the section
            // come out the same every time.
            //
            // This is not cosmetic. Re-converting a world after editing one block mapping is how the preview follows
            // an edit: the converter skips any chunk whose bytes already match what is on disk, and the viewer
            // re-renders only the chunks that were actually rewritten. Left to the scheduler, every section differed
            // from its own previous output on every run, so nothing could ever be recognised as unchanged and the
            // whole map was re-rendered regardless of how little had changed.
            List<TagWithName<?>> ordered = new ArrayList<>(outputs);
            ordered.sort(java.util.Comparator.comparing(TagWithName::name));
            for (TagWithName<?> tag : ordered) {
